...
# CORS: el navegador pregunta al Gateway, no al BFF. Se incluye la propia
# Invoke URL: desde gateway-frontend.ps1 la app se sirve por ahi (HTTPS).
$invokeCors = (Aws apigatewayv2 get-api --api-id $apiId --query 'ApiEndpoint' --output text).Trim()
Aws apigatewayv2 update-api --api-id $apiId --cors-configuration "AllowOrigins=$invokeCors,$origen,http://localhost:4200,AllowMethods=GET,POST,PUT,DELETE,OPTIONS,AllowHeaders=Authorization,Content-Type,Accept,MaxAge=3600" | Out-Null
...
# Frontend de produccion apuntando al Gateway. Si gateway-frontend.ps1 ya lo
# dejo servido por HTTPS desde el Gateway, no se toca: volver a http://IP
# rompe el login (Azure no acepta redirect URI http para una SPA).
$envProd = Join-Path $root 'frontend-agrotrack\src\environments\environment.prod.ts'
if ((Get-Content $envProd -Raw) -match [regex]::Escape("redirectUri: '$invoke'")) {
  Write-Host '   environment.prod.ts ya sirve la app por el Gateway (HTTPS): sin cambios'
} else {
@"
/**
...
Write-Host "`nSi cambio el secreto o es la primera vez: .\subir.ps1 -Ip $appsIp -Stack apps"
Write-Host "Para servir la app por HTTPS desde el Gateway: .\gateway-frontend.ps1"
